...
public class ShoppingList3 {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case "4": // Sort the items in the Shopping List
				...
				break;
			case "5": // save added items to a file
	...
	public static int addItems(Scanner sIn, ArrayList<String> shoppingList) {
		...
		int ItemsAdded = 0;
		  ...
		  while (true) {
		   ...
		   if (colonIndex == -1) {
			   System.out.println("Invalid!!, Please input using format <item>:<amount> ");
			   continue;
		   }
		   String[] parts = item.split(":");
		   String items = parts[0].trim();
		   String amount = parts[1].trim();
		    shoppingList.add(items + ":" + amount);
		    System.out.println("'" + items + ":" + amount +"' has been added to the shopping list.");
	...
	public static void saveList(Scanner sIn, ArrayList<String> shoppingList, String fileName) {
		System.out.print("Are you sure you want to overwrite the file? (Y/N): ");
		...
		if (response.equals("Y")) {
			try {
				...
				writer.close();
				System.out.println("Shopping list is saved to " + fileName);
			...
			System.out.println("Invalid response, Shopping list not saved.");
		}
	}
